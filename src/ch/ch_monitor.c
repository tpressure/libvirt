...
virCHMonitorBuildCPUJson(virJSONValue *content, virDomainDef *vmdef)
{
    ...
    if (maxvcpus != 0 || nvcpus != 0) {
        ...
        if (virCHMonitorBuildCPUTopologyJson(topology, vmdef) == 0) {
            VIR_INFO("Using CPU topology: %d:%d:%d:%d", vmdef->cpu->sockets, vmdef->cpu->dies, vmdef->cpu->cores, vmdef->cpu->threads);
...
virCHMonitorBuildMemoryZonesJson(virJSONValue *content, virDomainDef *def)
{
    ...
    VIR_DEBUG("Found NUMA cell configuration. Creating %ld NUMA nodes for guest VM.", ncells);
    ...
    for (i = 0; i < ncells; i++) {
        ...
        if (hostNodeCount > 1) {
            virReportError(VIR_ERR_CONFIG_UNSUPPORTED,
                           _("%ld host nodes specified but Cloud Hypervisor only supports 1"),
                           hostNodeCount);
            return -1;
        }
