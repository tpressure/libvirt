                       ...
                       int domid)
{
    ...
    event = virDomainEventLifecycleNewFromObj(vm, VIR_DOMAIN_EVENT_STOPPED,
                                              eventReason);
    virCHProcessStop(driver, vm, stopReason, VIR_CH_PROCESS_STOP_FORCE);
    virObjectEventStateQueue(driver->domainEventState, event);
                  ...
                  void *opaque G_GNUC_UNUSED)
{
    ...
    driverConf = g_strdup_printf("%s/ch.conf", ch_driver->config->configFileDir);
                        ...
                        int cancelled)
{
    ...
    // hypervisor process
    virCHProcessStop(driver, vm, VIR_DOMAIN_SHUTOFF_MIGRATED, 0);
