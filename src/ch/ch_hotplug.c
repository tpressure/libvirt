                                        ...
                                        unsigned int flags)
{
    ...
    if (flags & VIR_DOMAIN_AFFECT_CONFIG) {
        ...
        if (!(devConfig = virDomainDeviceDefParse(xml, vmdef, driver->xmlopt,
                                                  NULL, parse_flags))) {
            return -1;
        }
    ...
    if (flags & VIR_DOMAIN_AFFECT_LIVE) {
        if (!(devLive = virDomainDeviceDefParse(xml, vm->def, driver->xmlopt,
                                                 NULL, parse_flags))) {
            virReportError(VIR_ERR_INTERNAL_ERROR, "%s",
        ...
        if (chDomainDetachDeviceLive(driver, vm, devLive) < 0) {
            virReportError(VIR_ERR_INTERNAL_ERROR, "%s",
